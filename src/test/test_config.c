...
static void
test_config_adding_default_fallback_dir_servers(void *arg)
{
  (void)arg;
  ...
  /* Assume we have no default fallback directories.
   * XX/teor - change this to some reasonable range when default fallbacks are
   * added to tor. (Check if they're > 50 and < 300?) */
  add_default_fallback_dir_servers();
  tt_assert(smartlist_len(router_get_fallback_dir_servers()) == 0);
...
struct testcase_t config_tests[] = {
  ...
  CONFIG_TEST(adding_default_fallback_dir_servers, TT_FORK),
