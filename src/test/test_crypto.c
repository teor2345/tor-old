...
static void
test_crypto_rand_failure_modes(void *arg)
{
  int rv = 0;
  (void)arg;
  ...
  /* Check random works */
  rv = crypto_rand_check_failure_mode_zero(crypto_rand_raw);
  tt_assert(rv == 0);
  rv = crypto_rand_check_failure_mode_zero(crypto_rand);
  tt_assert(rv == 0);

  rv = crypto_rand_check_failure_mode_identical(crypto_rand_raw);
  tt_assert(rv == 0);
  rv = crypto_rand_check_failure_mode_identical(crypto_rand);
  tt_assert(rv == 0);

  rv = crypto_rand_check_failure_mode_predict(crypto_rand_raw);
  tt_assert(rv == 0);
  rv = crypto_rand_check_failure_mode_predict(crypto_rand);
  tt_assert(rv == 0);
  ...
  /* Check that everything works if only one function is bad */
  memset(mock_random_data, 0, MOCK_RANDOM_DATA_SIZE);

  MOCK(crypto_rand_raw, mock_crypto_rand);
  rv = crypto_rand_check_failure_mode_zero(crypto_rand_raw);
  tt_assert(rv == -1);
  rv = crypto_rand_check_failure_mode_zero(crypto_rand);
  tt_assert(rv == 0);
...
struct testcase_t crypto_tests[] = {
  ...
  { "rand_failure_modes", test_crypto_rand_failure_modes, TT_FORK, NULL, NULL },
  END_OF_TESTCASES
};
