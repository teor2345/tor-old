    ...
    SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
      if (!circ->marked_for_close &&
        ...
        SMARTLIST_FOREACH(surviving_services, rend_service_t *, ptr, {
          if (rend_circuit_matches_digest(oc, (uint8_t *) ptr->pk_digest)) {
            keep_it = 1;
            break;
          }
        });
...
rend_service_del_ephemeral(const char *service_id)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    if (!circ->marked_for_close &&
      ...
      origin_circuit_t *oc = TO_ORIGIN_CIRCUIT(circ);
      tor_assert(oc->rend_data);
      if (!rend_circuit_matches_digest(oc, (uint8_t *) s->pk_digest)) {
        continue;
      }
...
count_intro_point_circuits(const rend_service_t *service)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    if (!circ->marked_for_close &&
      ...
      origin_circuit_t *oc = TO_ORIGIN_CIRCUIT(circ);
      if (oc->rend_data &&
          rend_circuit_matches_digest(oc, (uint8_t *) service->pk_digest)) {
        num_ipos++;
      }
    }
  }
