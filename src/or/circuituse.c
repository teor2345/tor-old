                              ...
                              int flags)
{
   ...
   * if cannibalized (and 3 hops long if new). So if HSServiceRendRouteLength
  ...
  int hs_route_len = get_options()->HSServiceRendRouteLength;
  if (purpose == CIRCUIT_PURPOSE_S_CONNECT_REND &&
      hs_route_len >= MIN_HS_SERVICE_REND_ROUTE_LEN &&
      hs_route_len != (DEFAULT_HS_SERVICE_REND_ROUTE_LEN + 1)) {
    need_specific_len = 1;
  }
