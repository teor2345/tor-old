 ...
 */
static const char *default_fallback_authorities[] = {
...
{
  int i;
  for (i=0; default_fallback_authorities[i]; i++) {
    if (parse_dir_fallback_line(default_fallback_authorities[i], 0)<0) {
      log_err(LD_BUG, "Couldn't parse internal FallbackDir line %s",
              default_fallback_authorities[i]);
    }
  }
}
  ...
  if (!strcmp(question, "config/names")) {
    ...
    *answer = smartlist_join_strings(sl, "", 0, NULL);
    ...
    if (fallback_lines_seen == 0) {
      /*
       * We didn't see any explicitly configured fallback authorities,
       * so add the defaults to the list manually.
       *
       * default_fallback_authorities is included earlier in this file and
       * is a const char ** NULL-terminated array of fallback config lines.
       */
      const char **i;

      for (i = default_fallback_authorities; *i != NULL; ++i) {
        char *val = esc_for_log(*i);
