 ...
 * [ XXX - should we copy the connection objects instead?
 *         if so, do we need to deep-copy them?
 *         but this discards any downcast data...
 *         In the meantime, I've made it CONNECTION_PRIVATE - teor ] */
STATIC smartlist_t *
...
/** Return a directory connection (if any) in <b>state</b> that is fetching
 * the item described by <b>purpose</b>/<b>resource</b>, otherwise return
 * NULL. */
dir_connection_t *
connection_dir_get_by_purpose_resource_and_state(int purpose,
                                                 const char *resource,
                                                 int state)
{
...
/** Return a directory connection (if any) in <b>state</b> that is fetching
 * the item described by <b>purpose</b>/<b>resource</b>. If there are none,
 * return an empty list. This list must be freed using smartlist_free,
 * but the pointers in it must not be freed.
 * Note that this list should not be cached, as the pointers in it can be
 * freed if their connections close.
 ...
 *         In the meantime, I've made it CONNECTION_PRIVATE - teor ] */
STATIC smartlist_t *
connection_dir_list_by_purpose_resource_and_state(int purpose,
                                                  const char *resource,
                                                  int state)
{
  smartlist_t *conns = get_connection_array();
