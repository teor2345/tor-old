...
rend_cache_store_v2_desc_as_dir(const char *desc)
{
  ...
  while (rend_parse_v2_service_descriptor(&parsed, desc_id, &intro_content,
    ...
    ssize_t n_intro_points = -1;
    const char *failure_reason = NULL;
    int has_existing_cache_entry = 0;

    /* Make sure there aren't any intro points before we start */
    ...
    if (intro_content && intro_size > 0) {
       ...
       * know if there is any auth. So we do the initial check from
      ...
      if (!fast_memcmpstart(intro_content, intro_size,
        ...
        if (parsed->intro_nodes) {
          SMARTLIST_FOREACH(parsed->intro_nodes, rend_intro_point_t *, intro,
                            rend_intro_point_free(intro););
          smartlist_free(parsed->intro_nodes);
          parsed->intro_nodes = NULL;
        }
      }
    } else {
      /* We know there are no intro points: there is no intro section */
      n_intro_points = 0;
    }

    /* Make sure we haven't added any intro points */
    tor_assert(!parsed->intro_nodes);
    ...
    /* A list of fields in the v2 descriptor is available at:
     * https://gitweb.torproject.org/torspec.git/tree/rend-spec.txt#n218
     *
     * The v2 onion address or permanent-id is in the descriptor but is not
     * parsed.
     * Use desc_id_base32 instead: there is one desc_id per service address,
     * per replica (x2), per day for no auth and basic auth.
     * For stealth auth, each client key has a different desc_id, per service
     * address, per replica, per day.
     */
 ...
 advance:
  ...
  size_t trailing_byte_count = 0;
  if (current_desc) {
    trailing_byte_count = strlen(current_desc);
  ...
  if (!number_parsed || trailing_byte_count) {
    control_event_privcount_hsdir_cache_stored(
                                          ...
                                          NULL, /* don't know v2 desc */
                                          NULL, /* not v3 */
                                          trailing_byte_count,
                                          -1, /* don't know intro points */
                                          -1 /* don't know intro point size */
                                          /* skip intro point details */
                                          );
  }
  if (!number_parsed) {
