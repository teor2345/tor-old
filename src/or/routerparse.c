...
router_parse_addr_policy_item_from_string,(const char *s, int assume_action))
{
  ...
  if ((tok->tp == K_ACCEPT6 || tok->tp == K_REJECT6) &&
    ...
    /* We can't return NULL here, because that would eliminate the entire
     * address policy block. Just ignore this one entry. */
    addr_policy_t ignored_policy;
    memcpy(&ignored_policy, r, sizeof(addr_policy_t));
    ignored_policy.is_canonical = 0;
    ignored_policy.is_ignored = 1;
    ...
    return addr_policy_get_canonical_entry(&ignored_policy);
  }
