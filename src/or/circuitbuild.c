...
choose_good_entry_server(uint8_t purpose, cpath_build_state_t *state)
{
   ...
   * unless this would exclude all nodes
   * (i.e. we're in an incredibly small tor network)
   * This is an incomplete fix, but is no worse than the previous behaviour,
   * and only applies to minimal tor networks (so it's no less secure) */
  /*XXXX025 use the using_as_guard flag to accomplish this.*/
  if (options->UseEntryGuards
      && (smartlist_len(nodelist_get_list())
          > smartlist_len(get_entry_guards()))) {
