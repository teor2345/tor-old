                             ...
                             int pref_only)
{
  ...
  if (node->ri && fascist_firewall_allows_ri(node->ri, fw_connection,
                                             pref_only)) {
    return 1;
  } else if (node->rs && fascist_firewall_allows_rs(node->rs, fw_connection,
                                                    pref_only)) {
    return 1;
  } else if (node->md && fascist_firewall_allows_md(node->md, fw_connection,
                                                    pref_only)) {
    return 1;
  } else {
                                     ...
                                     int pref_only, tor_addr_port_t *ap)
{
  ...
  /* Sometimes, the rs is missing the IPv6 address info, and we need to go
   * all the way to the md */
  if (node->ri && fascist_firewall_choose_address_ri(node->ri, fw_connection,
                                                     pref_only, ap)) {
    return 1;
  } else if (node->rs && fascist_firewall_choose_address_rs(node->rs,
                                                            fw_connection,
                                                            pref_only, ap)) {
    return 1;
  } else if (node->md && fascist_firewall_choose_address_md(node->md,
                                                            fw_connection,
                                                            pref_only, ap)) {
    return 1;
  } else {
