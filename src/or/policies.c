...
/** Return true iff we think our firewall will let us make an OR connection to
 * <b>rs</b> on either its IPv4 or IPv6 address. */
int
fascist_firewall_allows_rs(const routerstatus_t *rs)
{
  if (fascist_firewall_allows_ipv4h_address_or(rs->addr,
                                               rs->or_port)) {
    return 1;
  }

  if (fascist_firewall_allows_address_or(&rs->ipv6_addr,
                                         rs->ipv6_orport)) {
    return 1;
  }
...
{
  if (fascist_firewall_allows_address_or(&md->ipv6_addr,
                                         md->ipv6_orport)) {
    return 1;
  }
...
{
  if (node->ri) {
    return fascist_firewall_allows_ri(node->ri);
  } else if (node->rs) {
    return fascist_firewall_allows_rs(node->rs);
  } else if (node->md) {
    /* XX/teor - Can a node ever have only an md without a rs?
     *           Can the md be more recent than the ri/rs? */
    return fascist_firewall_allows_md(node->md);
  } else {
    return 1;
  }
}
