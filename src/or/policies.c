                                    ...
                                    int add_default_policy)
{
  ...
  if (rejectprivate) {
    ...
    if (reject_interface_addresses) {
      tor_addr_t public_address;
      char bufif[POLICY_BUF_LEN];

      /* Reject the first public IPv4 address on any interface,
       * but don't reject our published IPv4 address twice */
      get_interface_address6(LOG_INFO, AF_INET, &public_address);
      if (!tor_addr_eq_ipv4h(&public_address, local_address)) {
        tor_snprintf(bufif, sizeof(bufif), "reject %s:*",
                     fmt_addr(&public_address));
      ...
      if (ipv6_exit) {
        /* Reject the first public IPv6 address on any interface,
         * but don't reject our published IPv6 address (if any) twice */
        get_interface_address6(LOG_INFO, AF_INET6, &public_address);
        ...
        if (ipv6_local_address == NULL
            || !tor_addr_eq(ipv6_local_address, &public_address)) {
