                            ...
                            const get_handler_args_t *args)
{
  ...
  /* After the path prefix follows the blinded pubkey which we use to get the
   * descriptor from the cache. Skip the prefix and get the pubkey. */
  tor_assert(strstr(url, "/tor/hs/3/"));
...
STATIC int
parse_version_from_post(const char *url, const char *prefix)
{
  int ok;
  ...
  /* Extract the version of the request which is of the form:
   *    /tor/hs/<version>/<command>   */
  ...
  start += strlen(prefix);
  /* Now, go to the next seperator, the version should be in-between. */
  end = strchr(start, '/');
  if (!end) {
    goto err;
  }
  v_str = tor_strndup(start, end - start);
  version = tor_parse_ulong(v_str, 10, 0, INT_MAX, &ok, NULL);
  tor_free(v_str);
  if (!ok) {
    goto err;
  }
...
handle_post_hs_descriptor(const char *url, const char *body)
{
  ...
  version = parse_version_from_post(url, "/tor/hs/");
  if (version < 0) {
    goto err;
  }
  /* We have a valid version number, now make sure it's a publish request. */
  if (!strstr(url, "/publish")) {
    goto err;
  }
